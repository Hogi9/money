...
@section('content')
	<div class="pb-4">
		...
		<p class="text-base-content/70 mt-1">Edit an existing user account and update their information.</p>
	</div>
				...
				<div class="md:col-span-2">
					...
					<select id="roles" name="roles[]" multiple class="@error('roles') is-invalid @enderror hidden"
						...
						@foreach ($roles as $role)
							<option value="{{ $role->name }}" {{ in_array($role->name, old('roles', $currentRoles)) ? 'selected' : '' }}>
								{{ ucfirst($role->name) }}
							</option>
						@endforeach
					</select>
